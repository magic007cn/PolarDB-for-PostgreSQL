...
</code></pre></div><p>本地数据目录类似于 PostgreSQL 的数据目录，大多数目录和文件都是通过 <code>initdb</code> 生成的。随着数据库服务的运行，本地数据目录中会产生更多的本地文件，如临时文件、缓存文件、配置文件、日志文件等。其结构如下：</p><div class="language-bash" data-ext="sh" data-title="sh"><pre class="language-bash"><code>$ tree ./ <span class="token parameter variable">-L</span> <span class="token number">1</span>
./
├── base
├── current_logfiles
├── global
...
</code></pre></div><h2 id="polar-basebackup-备份工具" tabindex="-1"><a class="header-anchor" href="#polar-basebackup-备份工具"><span>polar_basebackup 备份工具</span></a></h2>`,20),g=a("code",null,"polar_basebackup",-1),f={href:"https://www.postgresql.org/docs/11/app-pgbasebackup.html",target:"_blank",rel:"noopener noreferrer"},y=a("code",null,"pg_basebackup",-1),S=a("code",null,"pg_basebackup",-1),P=a("code",null,"polar_basebackup",-1),x=a("code",null,"bin/",-1),D=o(`<p>该工具的主要功能是将一个运行中的 PolarDB for PostgreSQL 数据库的数据目录（包括本地数据目录和共享数据目录）备份到目标目录中。</p><div class="language-bash line-numbers-mode" data-ext="sh" data-title="sh"><pre class="language-bash"><code>polar_basebackup takes a base backup of a running PostgreSQL server.
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><code>polar_basebackup</code> 的参数及用法几乎和 <code>pg_basebackup</code> 一致，新增了以下与共享存储相关的参数：</p><ul><li><code>--polar_disk_home</code> / <code>--polar_host_id</code> / <code>--polar_storage_cluster_name</code>：这三个参数指定了用于存放备份共享数据的共享存储节点</li><li><code>--polardata</code>：该参数指定了备份共享存储节点上存放共享数据的路径；如不指定，则默认将共享数据备份到本地数据备份目录的 <code>polar_shared_data/</code> 路径下</li></ul><h2 id="备份并恢复一个-replica-节点" tabindex="-1"><a class="header-anchor" href="#备份并恢复一个-replica-节点"><span>备份并恢复一个 Replica 节点</span></a></h2><p>基础备份可用于搭建一个新的 Replica（RO）节点。如前文所述，一个正在运行中的 PolarDB for PostgreSQL 实例的数据文件分布在各计算节点的本地存储和存储节点的共享存储中。下面将说明如何使用 <code>polar_basebackup</code> 将实例的数据文件备份到一个本地磁盘上，并从这个备份上启动一个 Replica 节点。</p><h3 id="pfs-文件系统挂载" tabindex="-1"><a class="header-anchor" href="#pfs-文件系统挂载"><span>PFS 文件系统挂载</span></a></h3><p>首先，在将要部署 Replica 节点的机器上启动 PFSD 守护进程，挂载到正在运行中的共享存储的 PFS 文件系统上。后续启动的 Replica 节点将使用这个守护进程来访问共享存储。</p><div class="language-bash" data-ext="sh" data-title="sh"><pre class="language-bash"><code><span class="token function">sudo</span> /usr/local/polarstore/pfsd/bin/start_pfsd.sh <span class="token parameter variable">-p</span> nvme1n1 <span class="token parameter variable">-w</span> <span class="token number">2</span>
...
</code></pre></div><p>备份完成后，可以以这个备份目录作为本地数据目录，启动一个新的 Replica 节点。由于本地数据目录中不需要共享存储上已有的共享数据文件，所以删除掉本地数据目录中的 <code>polar_shared_data/</code> 目录：</p><div class="language-bash" data-ext="sh" data-title="sh"><pre class="language-bash"><code><span class="token function">rm</span> <span class="token parameter variable">-rf</span> ~/replica1/polar_shared_data
</code></pre></div><h3 id="重新配置-replica-节点" tabindex="-1"><a class="header-anchor" href="#重新配置-replica-节点"><span>重新配置 Replica 节点</span></a></h3><p>重新编辑 Replica 节点的配置文件 <code>~/replica1/postgresql.conf</code>：</p><div class="language-diff" data-ext="diff" data-title="diff"><pre class="language-diff"><code><span class="token deleted-sign deleted"><span class="token prefix deleted">-</span><span class="token line">polar_hostid=1
...
</code></pre></div><h3 id="replica-节点启动" tabindex="-1"><a class="header-anchor" href="#replica-节点启动"><span>Replica 节点启动</span></a></h3><p>启动 Replica 节点：</p><div class="language-bash" data-ext="sh" data-title="sh"><pre class="language-bash"><code>pg_ctl <span class="token parameter variable">-D</span> <span class="token environment constant">$HOME</span>/replica1 start
...
</code></pre></div><h3 id="备份数据到本地存储和共享存储" tabindex="-1"><a class="header-anchor" href="#备份数据到本地存储和共享存储"><span>备份数据到本地存储和共享存储</span></a></h3><p>在用于部署 Standby 节点的机器上执行备份，以 <code>~/standby</code> 作为本地数据目录，以 <code>/nvme2n1/shared_data</code> 作为共享存储目录：</p><div class="language-bash" data-ext="sh" data-title="sh"><pre class="language-bash"><code>polar_basebackup <span class="token punctuation">\\</span>
    <span class="token parameter variable">--host</span><span class="token operator">=</span><span class="token punctuation">[</span>Primary节点所在IP<span class="token punctuation">]</span> <span class="token punctuation">\\</span>
    <span class="token parameter variable">--port</span><span class="token operator">=</span><span class="token punctuation">[</span>Primary节点所在端口号<span class="token punctuation">]</span> <span class="token punctuation">\\</span>
...
</span></span></code></pre></div><p>在 Standby 节点的复制配置文件 <code>~/standby/recovery.conf</code> 中添加：</p><div class="language-diff" data-ext="diff" data-title="diff"><pre class="language-diff"><code><span class="token inserted-sign inserted"><span class="token prefix inserted">+</span><span class="token line">recovery_target_timeline = &#39;latest&#39;
